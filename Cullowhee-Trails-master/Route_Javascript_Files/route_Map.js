...
var currentPositionObject;
var listOfSegments = [];
var listOfWayPoints = [];
var listOfIndexes = [];
var geoxml3Doc;
var distanceWalked;
var trailType = decodeURI(getQuerystring("qryTrailType")).split(" ").join("_");
var systemName = decodeURI(getQuerystring("qryCatTitle")).split(" ").join("_");
var routeName = decodeURI(getQuerystring("qryRouteFile")).split(" ").join("_");
...
function drawScreen() {
    ...
    RenderRequestedKMLPolylines(trailType, systemName, routeName, myMap);
    myMap = ShowMapWithPolylines();
    loadListOfWayPoints();
}

function getDistanceInfo() {
    geoxml3Doc = getGeoxml3Document();
    listOfIndexes = getArrayOfRequestedIndexes();
    distanceWalked = DistanceWalked(currentPositionObject, geoxml3Doc, listOfIndexes);
}
...
function loadListOfWayPoints() {
    ...
    URL = "Waypoints/" + systemName + "/wayPoint_" + routeCode + ".js";
...
function cb_UpdateCurrentPositionMarker(positionObject) {
    ...
    currentPositionObject = [{ "lat": lat, "long": long }];
