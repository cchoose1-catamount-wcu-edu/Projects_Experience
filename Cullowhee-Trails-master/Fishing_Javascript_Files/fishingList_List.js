...
function displayListOfPOIs() {
    ...
    var poi_id, poi_lat, poi_long, poi_title, poi_listItem_URL, poi_length, poi_difficulty, pathFile;
    ...
    for (var arrayIndex = 0; arrayIndex < arrayOfPOIData.length; arrayIndex++) {
        ...
        poi_difficulty = arrayOfPOIData[arrayIndex].POI_Difficulty;
        poi_length = arrayOfPOIData[arrayIndex].POI_Length;
        pathFile = arrayOfPOIData[arrayIndex].POI_File;
        ...
        if ((poi_lat != 0) && (poi_long != 0)) {
            ...
            //Build list item
            display = display + buildListItem(poi_id, poi_title, length, difficulty, poi_lat, poi_long, distanceFromBase, pathFile);
...
//--------------------------------------------------------------------
function buildListItem(parPOI_ID, parPOI_Title, parPOI_Length, parPOI_Difficulty, parPOI_Lat, parPOI_Long, parDistanceFromBase, parPathFile) {
    ...
    if ((parDistanceFromBase !== null)) {
        ...
        display += "<li>" + '<a rel="external" href="' + poi_listItem_URL + '"><span class="title">' +
                    parPOI_Title + '</span><br/><span class="subTitle">' + parPOI_Length + '; ' + parPOI_Difficulty + '</span><span class="distance">' + distanceDisplay + "</span></a></li>";
    }
    else {
